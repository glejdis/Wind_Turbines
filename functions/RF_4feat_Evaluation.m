...
for i = 1:length(testBins)
    bins = testBins(i);    
    [trainingFeatures, trainingLabels, testFeatures, testLabels, RFModel] = randomForest_4feat(bins, trainingSamples,mixed);
...
title('RF (4 features) 70% Training data, not mixed', 'Fontweight', 'bold', 'Fontsize', 18);
savefig('figures/RF/PrecRecNotMixed_4F')

figure;
confusionchart(confusionMatr{2},["healthy","broken"]);
title('RF (2features) Confusionmatrix 2 bins not mixed');
savefig('figures/RF/confusionMatrs2bins_4F');
...
title('RF (4 features) 70% Training data, not mixed', 'Fontweight', 'bold', 'Fontsize', 18);
savefig('figures/RF/NotMixedEval_4F');
...
grid on;
legend('Recall', 'Precision', 'Specificity');
title('RF (4 features) 70% Training data, mixed', 'Fontweight', 'bold', 'Fontsize', 18);
savefig('figures/RF/PrecRecMixed_4F')

figure;
confusionchart(confusionMatr{2},["healthy","broken"]);
title('RF (2features) Confusionmatrix 2 bins mixed');
savefig('figures/RF/confusionMatrs2binsMixed_4F');
...
legend('10 fold, 30% Holdout CrossValidation error');
title('RF (4 features) 70% Training data, mixed', 'Fontweight', 'bold', 'Fontsize', 18);
savefig('figures/RF/MixedEval_4F');
...
grid on
title('ROC Curve RF (4 features) for different bin Sizes', 'Fontweight', 'bold', 'Fontsize', 18);
savefig('figures/RF/ROC_4F');
...
title('AUC RF (4 features) for different bin Sizes', 'Fontweight', 'bold', 'Fontsize', 18);
grid on
end
